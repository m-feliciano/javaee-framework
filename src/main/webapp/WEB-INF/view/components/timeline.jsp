...
<div class="timeline-dashboard">
    <c:choose>
        ...
        <c:otherwise>
            <%-- Calculate all statistics and build chart data in a single loop --%>
            ...
            <div class="timeline">
                ...
                <c:forEach items="${activities}" var="activity" varStatus="status">
                    <fmt:formatDate value="${activity.timestamp}" pattern="dd/MM/yyyy" var="currentDate"/>
                    <fmt:formatDate value="${activity.timestamp}" pattern="HH:mm:ss" var="currentTime"/>
                    ...
                    <!-- Timeline Item -->
                    <div class="timeline-item status-${activity.status}">
                        ...
                        <div class="timeline-content">
                            <div class="timeline-header">
                                <div class="timeline-title-group">
                                    <h5 class="timeline-action">${activity.action}</h5>
                                ...
                                <div class="timeline-badges">
                                    <c:if test="${not empty activity.httpMethod}">
                                        <span class="badge badge-method badge-method-${fn:toLowerCase(activity.httpMethod)}">
                                            ${activity.httpMethod}
                                        </span>
                                    </c:if>
                                    <span class="badge badge-status badge-status-${activity.status}">
                                        ${activity.status}
                                    </span>
                                    <c:if test="${not empty activity.httpStatusCode}">
                                        <span class="badge badge-http-code">
                            ...
                            <div class="timeline-details">
                                <c:if test="${not empty activity.endpoint}">
                                    <div class="timeline-detail-item">
                                        ...
                                        <code class="timeline-endpoint">${activity.endpoint}</code>
                                    </div>
                                </c:if>
                                ...
                                <c:if test="${not empty activity.entityType}">
                                    <div class="timeline-detail-item">
                                        ...
                                        <c:if test="${not empty activity.entityId}">
                                            <code class="entity-id">#${fn:substring(activity.entityId, 0, 8)}</code>
                                        </c:if>
                                    </div>
                                </c:if>
