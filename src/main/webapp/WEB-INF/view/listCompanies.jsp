...
<div class="container">
	<div class="d-flex flex-row-reverse mb-2">
		<a type="button" href="${ newCompany }" class="btn btn-dark">Add
			Company</a>
	</div>
				...
				<table class="table table-striped table-bordered table-hover">
					...
					<tbody>
						<c:forEach items="${ companies }" var="company">
							<fmt:formatDate value="${company.releaseDate }"
								pattern="dd/MM/yyyy" var="releaseDate" />
							<tr>
								<th width="10%" scope="row">${ company.id }</th>
								<td width=30%>${ company.name }</td>
								<td width=30%>${ releaseDate }</td>
								<td width=20%><a type="button" class="btn btn-primary"><i
										class="bi bi-eye"></i></a> <a type="button"
									href="${ listCompany }&id=${ company.id }"
									class="btn btn-success"><i class="bi bi-pencil-square"></i></a>
									<a type="button" href="${ deleteCompany }&id=${ company.id }"
