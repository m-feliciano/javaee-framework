...
public class UserController extends BaseController {
    ...
    @RequestMapping(
            ...
            validators = {
                    ...
                    @Validator(values = "login", constraints = @Constraints(isEmail = true, message = "Login must be a valid email")),
                    @Validator(values = "password", constraints = @Constraints(minLength = 5, maxLength = 30, message = "Password length must be between {0} and {1} characters"))
            })
    @SneakyThrows
    ...
    @RequestMapping(
            ...
            validators = {
                    @Validator(values = "login", constraints = @Constraints(isEmail = true, message = "Login must be a valid email")),
                    @Validator(values = {"password", "confirmPassword"}, constraints = {
