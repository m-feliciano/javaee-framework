...
public class InventoryController extends BaseController {
    ...
    @RequestMapping(
            ...
            validators = {
                    ...
                    @Validator(values = "productId", constraints = @Constraints(notNullOrEmpty = true, message = "Product ID is required"))
            })
    ...
    @RequestMapping(
            ...
            method = RequestMethod.POST,
            validators = {
                    @Validator(values = "id", constraints = @Constraints(min = 1, message = "ID must be greater than or equal to {0}")),
                    @Validator(values = "description", constraints = {
                            @Constraints(minLength = 5, message = "Description must be between {0} and {1} characters")
                    }),
                    @Validator(values = "quantity", constraints = {
