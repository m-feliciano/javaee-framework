...
public class CategoryController extends BaseController {
    ...
    @RequestMapping(
            ...
            validators = @Validator(values = "id", constraints = @Constraints(min = 1, message = "ID is required"))
    )
    @SneakyThrows
    ...
    @RequestMapping(
            ...
            validators = @Validator(
                    values = "name",
                    constraints = @Constraints(minLength = 3, maxLength = 50, message = "Name must be between {0} and {1} characters")
            ))
    @SneakyThrows
