...
class DeleteProductServiceTest {
    ...
    void setUp() {
        ...
        lenient().when(hasInventoryUseCase.hasInventory(any(Inventory.class), eq(AUTH_TOKEN))).thenReturn(false);
    ...
    class SuccessfulDeletionTests {
        ...
        void shouldDeleteProductSuccessfully() {
            // Act
            deleteProductService.delete(productRequest, AUTH_TOKEN);
            ...
            verify(hasInventoryUseCase).hasInventory(any(Inventory.class), eq(AUTH_TOKEN));
...
//            verify(storageService).deleteFile(THUMB_URL);
    ...
    class ProductNotFoundTests {
        ...
        void shouldThrowExceptionWhenProductNotFound() {
            ...
            // Act & Assert
            assertThatThrownBy(() -> deleteProductService.delete(productRequest, AUTH_TOKEN))
    ...
    class InventoryValidationTests {
        ...
        void shouldThrowExceptionWhenHasInventory() {
            // Arrange
            when(hasInventoryUseCase.hasInventory(any(Inventory.class), eq(AUTH_TOKEN))).thenReturn(true);
    ...
    class ExecutionOrderTests {
        ...
        void shouldExecuteInCorrectOrder() {
            ...
            // Assert - Verify order
            var inOrder = inOrder(authenticationPort, productMapper, repositoryPort, hasInventoryUseCase, storageService);
            ...
            inOrder.verify(hasInventoryUseCase).hasInventory(any(Inventory.class), eq(AUTH_TOKEN));
...
//            inOrder.verify(storageService).deleteFile(THUMB_URL);
        ...
        void shouldCheckInventoryBeforeDeleting() {
            ...
            // Assert
            var inOrder = inOrder(hasInventoryUseCase, repositoryPort);
            inOrder.verify(hasInventoryUseCase).hasInventory(any(Inventory.class), eq(AUTH_TOKEN));
